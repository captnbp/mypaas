...
short_description: Manage OVH Cloud public keys
description:
    - Manage OVH (French European hosting provider) Cloud public keys
requirements: [ "ovh" ]
options:
    name:
        required: true
...
def main():
    ...
    # Check if the sshkey exists
    sshkeys = client.get('/cloud/project/{}/sshkey'.format(project_id))
    sshkey_exist = False
    sshkey_identical = False
    for sshkey in sshkeys:
        sshkey = client.get('/cloud/project/{}/sshkey/{}'.format(project_id, sshkey['id']))
        if name == sshkey['name']:
            sshkey_exist = True
            if publicKey == sshkey['publicKey']:
                sshkey_identical = True
                break
            else:
                break
    ...
    if state == 'absent':
        if not sshkey_exist:
            ...
            # Remove the sshkey
            client.delete('/cloud/project/{}/sshkey/{}'.format(project_id, sshkey['id']))
            module.exit_json(changed=True)

    # Add / modify a sshkey (modify doesn't handle region update)
    if state == 'present':
        if not sshkey_exist:
            client.post('/cloud/project/{}/sshkey'.format(project_id), name=name, publicKey=publicKey, region=region)
            module.exit_json(changed=True)
        elif sshkey_exist and not sshkey_identical:
            client.delete('/cloud/project/{}/sshkey/{}'.format(project_id, sshkey['id']))
            client.post('/cloud/project/{}/sshkey'.format(project_id), name=name, publicKey=publicKey, region=region)
            module.exit_json(changed=True)
        elif sshkey_exist and sshkey_identical:
            module.exit_json(changed=False)
